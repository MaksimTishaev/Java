    ...
    private void jButtonFindActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonFindActionPerformed
        ...
        if (row != -1) {
            ...
            if ((b - a) % h < 0.0001 || (b - a) % h > h - 0.0001) {
                LinkedList<RecIntegral> RecIntList = new LinkedList<RecIntegral>();
                LinkedList<ThreadIntegral> ThreadList = new LinkedList<ThreadIntegral>();
                int n = 2;
                double i = 0, hs, ha, hb;
                hs = (b - a) / n;
                while (hs % h >= 0.00001 && hs % h <= h - 0.00001) {
                    n = n - 1;
                    hs = (b - a) / n;
                }
                for (ha = a; ha < b; ha += hs) {
                    hb = ha + hs;
                    RecIntList.addLast(new RecIntegral(ha, hb, h));
                }
                for (RecIntegral Rec: RecIntList) {
                    i += 1;
                    ThreadList.addLast(new ThreadIntegral("ThreadIntegral " + i, Rec));
                }
                for (ThreadIntegral Thread: ThreadList) {
                    Thread.start();
                }
                try {
                    for (ThreadIntegral Thread: ThreadList) {
